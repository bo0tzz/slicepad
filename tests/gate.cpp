...
int main(int argc, char **argv)
{
    ...
    // Gate 16: the per-segment description a layer view is built from. The
    ...
    if (failures == 0) {
        sp_engine *probe = sp_engine_create(fixtures.c_str(), work.c_str());
        if (probe == nullptr) {
            failures += fail("gate16", "could not create an engine");
        } else {
            const std::string profile = fixtures + "/model.3mf";
            const std::string mesh = fixtures + "/model-shapr3d.3mf";
            ...
            if (sp_load_config(probe, profile.c_str()) != SP_OK ||
                ...
                sp_auto_orient(probe) != SP_OK || sp_arrange(probe) != SP_OK ||
                sp_slice(probe, path.c_str(), nullptr, nullptr) != SP_OK) {
                failures += fail("gate16", std::string("setup: ") + sp_last_error(probe));
            } else {
                ...
                if (count == 0 || roles == nullptr || layers == nullptr ||
                    ...
                    failures += fail("gate16", "a slice produced no per-segment description");
                } else {
                    ...
                    for (size_t i = 0; i < count && failures == 0; ++i) {
                        if (layers[i] < previous)
                            failures += fail("gate16", "layer index falls from " +
                    ...
                    if (failures == 0 && size_t(highest) + 1 != layer_count)
                        failures += fail("gate16", "layers run to " + std::to_string(highest) +
                                                       " but the count says " +
                    ...
                    if (failures == 0 && reported_layers != layer_count)
                        failures += fail("gate16", "toolpath has " + std::to_string(layer_count) +
                    ...
                    for (const auto &[role, times] : role_counts) {
                        if (failures == 0 && allowed.count(role) == 0)
                            failures += fail("gate16", "role " + std::to_string(int(role)) +
                    ...
                    for (unsigned char role : {SP_ROLE_OUTER_WALL, SP_ROLE_INNER_WALL,
                        ...
                        if (failures == 0 && role_counts.count(role) == 0)
                            failures += fail("gate16", "no segments of role " +
                    ...
                    for (size_t i = 0; i < count && failures == 0; ++i) {
                        if (!(widths[i] > 0.0f) || !(heights[i] > 0.0f)) {
                            failures += fail("gate16", "segment " + std::to_string(i) +
                    ...
                    if (failures == 0 && layer_height_at == config.end()) {
                        failures += fail("gate16", "the profile has no layer_height to check against");
                    } else if (failures == 0) {
                        ...
                        if (std::fabs(typical - configured) / configured > 0.15)
                            failures += fail("gate16", "most segments are " +
                    ...
                    if (failures == 0 && widest - narrowest < 0.01f)
                        failures += fail("gate16", "every segment is " + std::to_string(widest) +
                                                       "mm wide, which no real slice is");

                    if (failures == 0)
                        reported("gate16"), std::printf(
                            "gate16 segments: %zu across %zu layers, %zu roles, widths %.2f-%.2fmm\n",
    ...
    constexpr int kExpectedGates = 16;
